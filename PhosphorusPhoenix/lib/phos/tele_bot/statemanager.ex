defmodule Phos.TeleBot.StateManager do
  ...
  def get_state(user_id), do: Agent.get(__MODULE__, &Map.get(&1, user_id |> String.to_integer()))

  def set_state(user_id, struct) when is_integer(user_id), do: Agent.update(__MODULE__, &Map.put(&1, user_id, struct))
  def set_state(user_id, struct), do: Agent.update(__MODULE__, &Map.put(&1, user_id |> String.to_integer(), struct))
