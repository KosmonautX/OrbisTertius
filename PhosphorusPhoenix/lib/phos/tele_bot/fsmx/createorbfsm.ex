defmodule Phos.TeleBot.CreateOrbFSM do
  ...
  def before_transition(%{data: %{orb: %{payload: %{inner_title: inner_title}}}} = struct, "description", "location"), do: print_location_prompt(struct)
  def before_transition(struct, "media", "location"), do: print_location_prompt(struct)
  def before_transition(%{data: %{location_type: type, orb: %{central_geohash: geohash}}} = struct, "location", "media"), do: print_media_prompt(struct)
  def before_transition(struct, "preview", "media"), do: print_media_prompt(struct)
  def before_transition(%{data: %{location_type: type, orb: %{central_geohash: geohash, payload: %{inner_title: inner_title}}}} = struct, _initial_state, "preview"), do: print_preview_prompt(struct)

  defp print_description_prompt(struct) do
  ...
  defp print_location_prompt(struct) do
  ...
  defp print_currentlocation_prompt(struct) do
  ...
  defp print_media_prompt(struct) do
  ...
  defp print_preview_prompt(struct) do
    {:ok, user} = BotCore.get_user_by_telegram(struct.telegram_id)
    {:ok, %{branch: %{data: %{orb: orb, media: %{media: media}} = data} = branch}} = StateManager.get_state(struct.telegram_id)
    ...
    if Enum.empty?(media) do
      ...
      ExGram.send_photo(struct.telegram_id, Phos.Orbject.S3.get!("ORB", orb.id , "public/profile/lossless"), caption: Template.orb_creation_preview_builder(data),
        parse_mode: "HTML", reply_markup: Button.build_createorb_preview_inlinekeyboard())
