defmodule PhosWeb.Components.List do
  ...
  def render(assigns) do
    if Map.get(assigns, :value) != nil do
      ...
      <div class="w-full flex items-center justify-between px-4 py-2 border-b">
        ...
        <div class="w-1/2">
          <%= if(Map.get(assigns, :editable, false), do: editable_value(assigns), else: @value) %>
        </div>
      </div>
      """
    else
  ...
  @impl true
  def handle_event("update", params, %{assigns: %{update: update, data: data}} = socket)
      when is_function(update) do
    socket = assign(socket, :disabled, true)

    case apply(update, [data, params]) do
      true ->
        {:noreply, put_flash(socket, :info, "data updated successfully")}

      {:ok, msg} when is_bitstring(msg) ->
        {:noreply, put_flash(socket, :info, msg)}

      {:ok, data} when is_map(data) ->
        {:noreply, put_flash(socket, :info, "data updated successfully") |> assign(:data, data)}

      {:error, msg} ->
        {:noreply, put_flash(socket, :error, msg)}

      _ ->
        {:noreply, put_flash(socket, :error, "data failed to update")}
    end
  end
