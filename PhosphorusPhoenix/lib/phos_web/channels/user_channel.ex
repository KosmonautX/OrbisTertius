...
000  # Channels can be used in a request/response fashion
  ...
  def handle_info(:initiation,  socket) do
    ...
    if (Phos.Repo.get_by(Phos.Users.User, fyr_id: socket.assigns.user_channel_id) == nil) do
      ...
      Phos.Users.create_user(%{"username" => user_payload["payload"]["username"], "fyr_id" => socket.assigns.user_channel_id, "media" => user_payload["payload"]["media"], "public_profile" => %{"birthday" => user_payload["payload"]["birthday"], "bio" => user_payload["payload"]["bio"]}, "profile_pic" => user_payload["payload"]["profile_pic"] , "private_profile" => %{"geohash" => geo_map}})
