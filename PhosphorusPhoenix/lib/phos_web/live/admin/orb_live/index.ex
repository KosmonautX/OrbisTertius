defmodule PhosWeb.Admin.OrbLive.Index do
  ...
  def handle_params(
        %{"page" => page} = _params,
        _url,
        %{assigns: %{traits: traits, pagination: pagination, limit: limit} = _assigns} = socket
      ) do
  ...
  defp filter_by_traits(keyword, options) when is_list(keyword),
    do: Action.filter_orbs_by_traits(keyword, options)
  ...
  def table_values(%{entries: []} = assigns) do
    ...
    <tr>
      <td
        colspan="6"
        class="border-t-0 px-6 align-middle text-center border-l-0 border-r-0 text-xs italic whitespace-nowrap p-4 text-left"
      >
        No Orbs found
      </td>
    </tr>
